...
namespace QuartzInAction.DAL
{
    public class DBConfigDAL
    {
        ...
        /// DBConfigModel实体类 
        /// </summary>
        protected QuartzInAction.Model.DBConfigModel Populate_BankList(IDataReader reader,
            Dictionary<string, string> fileds, string _conStr)
        {
            ...
            if (fileds.ContainsKey("DBName") && !Convert.IsDBNull(reader["DBName"]))
                model.DBName = !string.IsNullOrEmpty(reader["DBName"].ToString())
                    ? reader["DBName"].ToString().Trim()
                    : string.Empty;

            // 
            if (fileds.ContainsKey("DBNum") && !Convert.IsDBNull(reader["DBNum"]))
                model.DBNum = !string.IsNullOrEmpty(reader["DBNum"].ToString())
                    ? reader["DBNum"].ToString().Trim()
                    : string.Empty;
