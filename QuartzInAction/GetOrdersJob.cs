                ...
                foreach (DBConfigModel dbConfigModel in dbConfigLists)
                {
                    ...
                    try
                    {
                        ...
                        if (flag)
                        {
                            ...
                            IList<OrderModel> orderList = new OrderDAL().GetEntities(ordersql, conStr); //获取各服务器相同条件下的一批订单，因为订单数据(如 订单状态，收货人电话)是不断变化，只能不断轮询拉取，更新到数据中心
                            ...
                            for (int i = 0; i < orderList.Count; i++)
                            {
                                ...
                                //订单对应的具体物流信息从表
                                if (insertFlag && !string.IsNullOrEmpty(courierNum) &&!courierNum.Equals("11111111111") && !courierNum.Equals("22222222111"))
                                {
                                    #region 物流批处理
                                    ...
                                    //获取快递状态 order by id 
                                    string getStatusSql = string.Format( "select * from ExpressStatus where nu='{0}' order by id  ",  courierNum);
                                    ...
                                    IList<ExpressStatusModel> deliveryList = ExpressStatusDt.ToLists<ExpressStatusModel>();
